let json = `[{
    ...
    "name": "Batman",
    "universe": "DC Comics",
    "alterEgo": "Bruce Waine",
    "job": "fights evil, philanthropist, milliarder",
    "friends": "Robin, Batgirl",
    "superPowers": "интеллект, обширные познания, знания боевых искусств, ловкость",
    "moreInfor": "по популярности человек - летучая мышь может сравниться только с Суперменом.Его образ кажется очень мистическим и мрачным: черный костюм с развевающимся плащом, устрашающий Бэтмобиль, штаб - квартира в сырой пещере.Его биография настолько же темна, как и образ.В детстве у него на глазах убили родителей, и юный Брюс поклялся всеми силами защищать родной Готэм от преступности.
...
document.addEventListener("DOMContentLoaded", function (event) {
            ...
            let superheroDescription = "";

            for (let hero of superheroes) {
                superheroDescription += `<div class="superheroDescription">
        <img class="img" alt="picture" src='${hero.image}'>
        <h2> ${hero.name}</h2>
        <h3> Universe: ${hero.universe}</h3>
        <h3> Alter Ego: ${hero.alterEgo}</h3>
        <h3> Job: ${hero.job}</h3>
        <h3> Friends: ${hero.friends}</h3>
        <h4> Superpowers: ${hero.superPowers}</h4>
        <h4 class="text"> More information: ${hero.moreInfo}</h4>
        <input type='number' class='mark' id=${finalName}>
        <button class="btnAction" name=${finalName} id=${i} >Give your score</button>
        </div>`


                document.getElementById("container").innerHTML = superheroDescription;
                for (let i = 0; i < superheroes.length; i++) {
                    let btn = document.getElementById(i);
                    btn.addEventListener("click", function () {
                                let name = btn.name;
                                let mark = document.getElementById(name).value;

                                if (localStorage.getItem(name) == null) {
                                    localStorage.setItem(name, mark);
                                } else {
                                    localStorage.getItem(name) = mark;
                                }
                            });
                        }
                    }
